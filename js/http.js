...
HttpServer.prototype = {
  ...
  listen: function(port, opt_host) {
    var t = this;
    socket.tcpServer.create({}, function(socketInfo) {
      t.socketInfo_ = socketInfo;
      socket.tcpServer.listen(t.socketInfo_.socketId, opt_host || '0.0.0.0', port, 50, function(result) {
        t.readyState_ = 1;
        socket.tcpServer.onAccept.addListener(function(acceptInfo) {
          if (acceptInfo.socketId === t.socketInfo_.socketId) {
            chrome.sockets.tcp.setPaused(acceptInfo.clientSocketId, false);
          }
        });
        socket.tcp.onReceive.addListener(t.readRequestFromSocket_.bind(t));
      });
    });

  },

  readRequestFromSocket_: function(readInfo) {
    var requestData = '';
    var endIndex = 0;
    ...
    var request = new HttpRequest(headerMap, readInfo.socketId);
  ...
  close: function(){
    socket.tcpServer.disconnect(this.socketInfo_.socketId);
    socket.tcpServer.close(this.socketInfo_.socketId);
  }
};
...
HttpRequest.prototype = {
  ...
  close: function() {
    ...
    if (this.headers['Connection'] != 'keep-alive') {
      socket.tcp.disconnect(this.socketId_);
      socket.tcp.close(this.socketId_);
    }
    this.socketId_ = 0;
    this.readyState = 3;
  },
  ...
  write_: function(array) {
    ...
    console.log('write');
    socket.tcp.send(this.socketId_, array, function(writeInfo) {
...
WebSocketServerSocket.prototype = {
  ...
  readFromSocket_: function() {
    ...
    socket.tcp.onReceive.addListener(onDataRead);
  },
